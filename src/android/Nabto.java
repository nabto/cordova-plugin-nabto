...
public class Nabto extends CordovaPlugin {
    ...
    private Map<String, CordovaStream> streams;
    ...
    public Nabto() {
        ...
        streams = new HashMap<String, CordovaStream>();
    }
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        else if (action.equals("streamRead")) {
            streamRead(args.getString(0), args.getInt(1), callbackContext);
        }
        // Nabto Tunnel API
    ...
    private void streamOpen(final String host, final CallbackContext cc) {
        cordova.getThreadPool().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    CordovaStream stream = new CordovaStream(nabto);
                    NabtoStatus status = stream.open(host, session);
    ...
    private void streamClose(final String streamHandle, final CallbackContext cc) {
        cordova.getThreadPool().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    CordovaStream stream = streams.get(streamHandle);
                    if (stream != null) {
                        NabtoStatus status = stream.close();
                        if (status == NabtoStatus.OK) {
    ...
    private void streamRead(final String streamHandle, final int bytes, final CallbackContext cc) {
        cordova.getThreadPool().execute(new Runnable() {
                @Override
                public void run() {
                    if (nabto == null) {
                    ...
                    CordovaStream stream = streams.get(streamHandle);
                    if (stream == null) {
                        cc.error(NabtoStatus.STREAM_CLOSED.ordinal());
                        return;
                    } else {
                        StreamReadResult result = stream.read(bytes);
                        if (result.getStatus() != NabtoStatus.OK) {
                            cc.error(result.getStatus().ordinal());
                            return;
                        } else {
                            String base64Encoded = Base64.encodeToString(result.getData(), Base64.DEFAULT);
                            cc.success(base64Encoded);
                            return;
                        }
                    }
                }
    ...
    private void streamWrite(final String streamHandle, final String data, final CallbackContext cc) {
        cordova.getThreadPool().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    CordovaStream stream = streams.get(streamHandle);
                    if (stream != null) {
                        NabtoStatus status = stream.write(Base64.decode(data, Base64.DEFAULT));
                        if (status == NabtoStatus.OK) {
                            cc.success();
                        } else {
                            cc.error(status.ordinal());
    ...
    private void streamConnectionType(final String streamHandle, final CallbackContext cc) {
        cordova.getThreadPool().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    CordovaStream stream = streams.get(streamHandle);
                    if (stream != null) {
                        NabtoConnectionType type = stream.connectionType();
